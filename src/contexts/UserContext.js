...
async function buildLink(id, vidTitle) {
  const link = await dynamicLinks().buildShortLink({
    ...
    android: {
      packageName: "com.samaira.mobiwood.android",
...
const UserContextProvider = ({ children }) => {
  ...
  const handleShare = (videoId, vidTitle, shares, updateVidShare) => {
    buildLink(videoId, vidTitle).then((link) => {
      // console.log(`${link}`);
      const title = `${vidTitle}`;
      // const message = 'Please check this out on the uDanta App "Your Home for News"\n';
      const message = `${`Hey! Check this out`} - ${vidTitle}\n`;
      ...
      Share.open(options)
        .then((res) => {
          ...
          firestore()
            ...
            .then(() => {
              updateVidShare({ id: videoId, shares: sharesCount });
            });
        })
        .catch((err) => {
          err && console.log(err);
  ...
  const updateFollowing = async (action, userId) => {
    ...
    await firestore()
      ...
      .then((resp) => {
        ...
        if (resp.data().followers) allFollowers = [...resp.data().followers];
      });
    ...
    console.log(
      `allFollowers before : ${JSON.stringify(allFollowers)}, myuid : ${uid}`
    );
    return firestore()
      .collection("user")
      .doc(userId)
