...
import { View,Text,Image, FlatList, TouchableOpacity, ToastAndroid, Platform, RefreshControl }from 'react-native'
              ...
              renderItem = {({item, index}) =>  (
                ...
                :null
                  ...
                  <TouchableOpacity onPress={() => {
                    ...
                    {
                      !item.profile?
                        <Image source={require('../assets/images/user-placeholder.png')} style={{width:28, height:28, marginLeft:10, marginTop:8, marginBottom:8, borderRadius:60, borderWidth:0, borderColor:'#bbb',}} />
                      :
                        <Image source={{uri:item.profile}} style={{width:28, height:28, marginLeft:10, marginTop:8, marginBottom:8, borderRadius:60, borderWidth:1, borderColor:'#bbb',}} />
                    }
                  </TouchableOpacity>
                    ...
                    <View>
                      <TouchableOpacity onPress={() => {
                       videoContext.setViewProfile(item.userid);
                       videoContext.setViewDisplayName(item.displayName); props.navigation.navigate('exploreProfile')}}>
                        <Text style={{marginTop:12, marginLeft:0, fontWeight:'bold', fontSize:15, position:'absolute', top:-45, left:45}}>
                    ...
                    <TouchableOpacity style={{position:'absolute', right:15, marginTop:15 }} onPress={() => props.reportModal(item.id, item, true)}>
                      <FeatherIcon
                  ...
                  <View style={{paddingLeft:10, marginTop:12, marginBottom:10, flexDirection:'row'}}>
                    <TouchableOpacity onPress={()=>{
                    ...
                    <Text style={{fontSize:17, marginRight:5}}><IconClose name={!videoContext.vidLikesMap.get(item.id)?'heart-outline':'heart'} size={20} color={!videoContext.vidLikesMap.get(item.id)?'black':'red'} /></Text>
                    <Text style={{fontSize:17}}>{videoContext.vidLikesMap.get(item.id)}</Text>
                    </View>
                  </TouchableOpacity>
                  <TouchableOpacity style={{marginLeft:0}} onPress={()=>{usrCntxt.handleShare(item.id, item.description)}}>
                    <Text style={{marginLeft:15, fontSize:17}} >
                    <IconClose  name='arrow-redo-outline' size={20} color='black' /> {item.shares?item.shares:0} </Text>
                  </TouchableOpacity>
                  <Text style={{marginLeft:15, fontSize:17,}}>
                    <IconClose name='eye-outline' size={20} color='black' /> {videoContext.noOfViewsMap.get(item.id)}</Text> 
                  ...
                  <Text style={{ fontWeight:'bold'}}>{!item.displayName?"MobiWood User":item.displayName}{" "}</Text>
                          {item.description}
                        </Text>
                        </View>
                </View>
