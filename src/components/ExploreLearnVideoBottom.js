...
const ExplorVideoBottom = props => {
    ...
    React.useEffect(()=>{
        if(props.searchVid)
        {
            let rslt = vidContext.videos.filter(data=>data.talent.match(props.searchKeyword.toLowerCase()) || (data.displayName && data.displayName.toLowerCase().includes(props.searchKeyword.toLowerCase())));
