...
import ImageGridRenderItem from "./ImageGridRenderItem.js";
...
export default function ImageGrid(props) {
  ...
  const onRefresh = React.useCallback(() => {
    setRefreshing(true);
    wait(1000).then(() => {
      videoContext.fetchLimitedVideos();
      setRefreshing(false);
    });
  }, []);

  const updateVidShare = (reslt) => {
    const { id, shares } = reslt;
    let tmp = new Map(videoContext.noOfSharesMap);

    tmp.set(id, shares);
    videoContext.setNoOfSharesMap(tmp);
  };
          ...
          <ImageGridRenderItem key={index}>
            <View style={styles.imgContainer}>
              ...
              <View style={{ flex: 0.7, marginTop: 5 }}>
                <ImageGridItem
                  item={item}
                  noOfViews={videoContext.noOfViewsMap.get(item.id)}
                  currentVisibleVideo={currentVisibleVideo}
                  myIndex={index}
                  setNoOfViewsMap={(views) => {
                    let tmpViewsMap = new Map(videoContext.noOfViewsMap);
                    tmpViewsMap.set(item.id, views);
                    videoContext.setNoOfViewsMap(tmpViewsMap);
                  }}
              ...
              >
                ...
                >
                  <View style={{ flexDirection: "row" }}>
                    ...
                    <Text style={{ fontSize: 17 }}>
                      {videoContext.vidLikesMap.get(item.id) || 0}{" "}
                    </Text>
                  </View>
                </TouchableOpacity>
                <TouchableOpacity
                ...
                >
                  <Text style={{ marginLeft: 15, fontSize: 17 }}>
                    <IconClose
                      ...
                      color="black"
                    />{" "}
                    {videoContext.noOfSharesMap.get(item.id) || 0}{" "}
                  </Text>
                </TouchableOpacity>
                <Text style={{ marginLeft: 15, fontSize: 17 }}>
                  <IconClose name="eye-outline" size={20} color="black" />{" "}
                  {videoContext.noOfViewsMap.get(item.id) || 0}
                </Text>
                <Text
                  style={{
                    right: 0,
                ...
                >
                  {/* @ {videoContext.uploadTime}{" "}
                  {new Date(videoContext.uploadTime).getDate()},
                  {month[new Date(videoContext.uploadTime).getMonth()]} */}
                </Text>
              </View>
