...
import {View, Text, Image, SafeAreaView, StyleSheet, ToastAndroid, FlatList, Dimensions, Modal, TouchableOpacity, Platform, Alert, ImageBackground} from 'react-native';
        ...
        <Modal
          ...
          >
            <View style={{justifyContent:'center',flex:1, backgroundColor:'black'}}>
                <View >
                  <TouchableOpacity onPress={()=>{setModalVisible(false)}} style={{backgroundColor:'grey', borderRadius:1000, borderWidth:2, borderColor:'black', position:'absolute', padding:5, zIndex:10, top:-30, right:10, alignSelf:'flex-end'}}>
                  ...
                  }} style={{padding:10, marginTop:10, backgroundColor:'red', alignSelf:'center', borderRadius:5}}>
                    <Text style={{color:'white'}}><IconClose name="trash-outline"
                      size={14}
                      color="white"></IconClose>  Delete Video</Text>
                  </TouchableOpacity>
                </View>
            </View>
            ...
            {userCont?
                ...
                <View style={{flex:1,}}>
                    <View style={{marginBottom:20,  backgroundColor:'white', width:'95%', alignSelf:'center', paddingBottom:22, borderBottomLeftRadius:20, borderBottomRightRadius:20}}>
                      <View style={{backgroundColor:'black',borderRadius:1000, width:100, height:100,alignSelf:'center', marginTop:20}}>
                        <Image
                            source={{uri: userCont.profilePhoto?userCont.profilePhoto:filePath.uri}}
                            style={{width:100, height:100,alignSelf:'center',borderRadius:1000,resizeMode:'cover',borderWidth:1}}
                        />

                    <TouchableOpacity
                          activeOpacity={0.5}
                          style={{backgroundColor:'black',width:25, height:25, padding:5.5, alignSelf:'flex-end',top:-windowHeight/20,borderRadius:200}}
                          onPress={chooseFile}>
                               <FeatherIcon name="plus" size={15} color={'white'} />
                        </TouchableOpacity>
                      </View>
                
                      <Text style={{fontSize:20,fontWeight:'700',alignSelf:'center', marginBottom:5,marginTop:5}}>{userDetails.providerData[0].displayName}</Text>
                        <View style={{flexDirection:'row',justifyContent:'space-around',marginTop:5}}>
                                <Text style={{fontSize:16,fontWeight:'600', backgroundColor:'#950a3e', color:'white', textAlign:'center', fontSize:14, padding:10, paddingLeft:10, paddingRight:0, borderTopLeftRadius:40,borderBottomLeftRadius:40, position:'absolute', left:'5%', width:'45%'}}><Text style={{fontWeight:'bold',fontSize:18}}>{userCont.followers&&userCont.followers.length!=0?userCont.followers.length:0}</Text>{"\n"} Followers</Text>
                                <Text style={{fontSize:16,fontWeight:'600', backgroundColor:'#b50351', color:'white', textAlign:'center', padding:10,fontSize:14,  paddingLeft:0, paddingRight:10, borderTopRightRadius:40,borderBottomRightRadius:40, position:'absolute', right:'5%', width:'45%'}}><Text style={{fontWeight:'bold',fontSize:18}}>{userCont.following&&userCont.following.length!=0?userCont.following.length:0}</Text>{"\n"} Following</Text>
                        </View>
                    </View>
                    ...
                    <FlatList 
                        ...
                        renderItem={({item, index})=>(
                            <View key={index}  style={styles.layoutA}>
                                {console.log(item)}
                                <TouchableOpacity onPress={()=>HandleClick(item.videoUrl,item.thumbnail, item.id)} style={{margin:1}} >
                                <Image
