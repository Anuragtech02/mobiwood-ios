...
const UploadScreen = (props) => {
  ...
  const compressVideo = async (filepath) => {
    ...
    let origin = {};
    try {
      origin = await ProcessingManager.getVideoInfo(path);
    } catch (err) {}
    console.log('origin', origin) 
    const result = await ProcessingManager.compress(path, {
      width: videoDetails.width || 720,
      height: videoDetails.height || 1280,
      bitrateMultiplier: 15,
      minimumBitrate: 800000,
    });
*/
    // const thumbnail = await ProcessingManager.getPreviewForSecond(result.source, 2000);
  ...
  const uploadVideo = async () => {
    ...
    setIsUploading(true);
    const filePathURI =
      Platform.OS === "android" ? filePath.path : filePath.uri.substr(7);
    const file = await compressVideo(filePathURI);
      ...
      <ScrollView style={{ flex: 1 }} showsVerticalScrollIndicator={false}>
        {!isUploading ? (
          <View>
              ...
              {filePath.uri && (
                <>
                  <Video
                    ...
                    onLoad={(e) => setVideoDetails(e.naturalSize)}
                  />
                </>
              )}
            </View>
            ...
            <View
              style={{ alignSelf: "center", marginTop: 25, paddingLeft: 0 }}
            >
              <Text
                style={([styles.label], { marginLeft: 0, marginBottom: 10 })}
              >
                Contest
              </Text>
              <DropDownPicker
                items={[
                  ...
                  { value: "Best Photo", label: "Best Photo" },
                ...
                zIndex={50000}
                defaultNull
                onChangeItem={(item) => setContest(item.value)}
              />

              <Text
                style={
                  ([styles.label],
                  { marginTop: 20, marginLeft: 0, marginBottom: 10 })
                }
              >
                Talent
              </Text>
              ...
              <View style={styles.checkboxContainer}>
                <Checkbox
                  ...
                  style={
                    ([styles.checkbox],
                    { width: 20, height: 20, border: "none", marginRight: 10 })
                  }
                  // lineWidth={10}
                />
                <Text style={{ marginTop: 2, marginRight: 10 }}>
                  {" "}
